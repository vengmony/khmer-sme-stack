...
export default function InvoiceForm({ mode, customers, items, defaultCurrency, nextNumber, initialCustomerId, initial }: Props) {
  ...
  function addItem(item: any) {
    if (lines.find(l => l.itemId === item.id)) {
      ...
      setLines([...lines, { itemId: item.id, name: item.name, sku: item.sku, quantity: 1, unitPrice: item.unitPrice, taxRate: 0 }]);
    }
    setSearch('');
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const url = mode === 'create' ? '/api/invoices' : `/api/invoices/${initial.id}`;
      ...
      <div className="card p-5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} required className="input" />
          </div>
          <div>

...
import { Save, Loader2, Plus, Trash2, X, Search } from 'lucide-react';
import { formatCurrency, round2, cn } from '@/lib/utils';

interface LineItem { itemId: string; name: string; sku: string; quantity: number; unitPrice: number; discount: number; taxRate: number; }

interface Props {
  mode: 'create' | 'edit';
  customers: { id: string; name: string; currencyCode: string }[];
  items: { id: string; sku: string; name: string; unitPrice: number; unit: string }[];
...
export default function SalesOrderForm({ mode, customers, items, warehouses, defaultCurrency, nextNumber, initialCustomerId, initial }: Props) {
  ...
  const [lines, setLines] = useState<LineItem[]>(initial?.items?.map((i: any) => ({
    ...
    unitPrice: i.unitPrice, discount: i.discount || 0, taxRate: i.taxRate || 0,
  })) || []);

  // When customer changes, switch currency
  useEffect(() => {
  ...
  function addItem(item: any) {
    if (lines.find(l => l.itemId === item.id)) {
      ...
      setLines([...lines, { itemId: item.id, name: item.name, sku: item.sku, quantity: 1, unitPrice: item.unitPrice, discount: 0, taxRate: 0 }]);
    }
    setSearch('');
  }
  ...
  const taxTotal = lines.reduce((s, l) => s + ((l.quantity * l.unitPrice - (l.discount || 0)) * (l.taxRate || 0) / 100), 0);
    ...
    try {
      const body = {
        ...
        items: lines.map(l => ({
          ...
          taxRate: l.taxRate, taxAmount: round2((l.quantity * l.unitPrice - l.discount) * l.taxRate / 100),
          total: round2((l.quantity * l.unitPrice - l.discount) * (1 + l.taxRate / 100)),
        })),
      };
      const url = mode === 'create' ? '/api/sales-orders' : `/api/sales-orders/${initial.id}`;
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="card p-5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select value={form.customerId} onChange={(e) => setForm({ ...form, customerId: e.target.value })} required className="input">
          ...
          <div>
            ...
            <input value={form.orderNumber} onChange={(e) => setForm({ ...form, orderNumber: e.target.value })} required className="input" />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={form.currencyCode} onChange={(e) => setForm({ ...form, currencyCode: e.target.value })} className="input">
              {['USD', 'EUR', 'GBP', 'JPY', 'CNY', 'INR', 'AUD', 'CAD', 'SGD'].map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="card p-5 space-y-3">
          ...
          {search && (
            <div className="absolute z-10 left-0 right-0 mt-1 bg-white border border-ink-200 rounded-md shadow-lg max-h-72 overflow-y-auto">
              {filteredItems.length === 0 ? (
                <div className="p-3 text-sm text-ink-500">No items found</div>
              ) : filteredItems.map(i => (
                <button
                  key={i.id} type="button" onClick={() => addItem(i)}
                  className="w-full px-3 py-2 text-left hover:bg-ink-50 flex items-center justify-between text-sm border-b border-ink-100 last:border-0"
                >
                  <div>
                    <p className="font-medium">{i.name}</p>
                    <p className="text-xs text-ink-500 font-mono">{i.sku}</p>
                  </div>
                  <p className="text-sm font-medium">{formatCurrency(i.unitPrice)}</p>
                </button>
              ))}
            </div>
          )}
        ...
        {lines.length === 0 ? (
          <div className="text-center py-8 text-sm text-ink-500 border border-dashed border-ink-200 rounded-md">
            Search and click an item above to add it.
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="text-xs text-ink-500 border-b border-ink-200">
                <tr>
                  ...
                  <th className="text-right py-2 font-medium w-24">Discount</th>
              ...
              <tbody className="divide-y divide-ink-100">
                {lines.map((l, idx) => {
                  const lineSub = l.quantity * l.unitPrice - (l.discount || 0);
                  const lineTotal = lineSub * (1 + (l.taxRate || 0) / 100);
                  return (
                    <tr key={idx}>
                      <td className="py-2">
                        <p className="font-medium text-sm">{l.name}</p>
                        <p className="text-xs text-ink-500 font-mono">{l.sku}</p>
                      </td>
                      ...
                      <td className="py-1.5"><input type="number" min="0" step="0.01" value={l.discount} onChange={(e) => updateLine(idx, { discount: parseFloat(e.target.value) || 0 })} className="input text-right" /></td>
                      ...
                      <td className="py-2 text-right font-medium">{formatCurrency(lineTotal, form.currencyCode)}</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5 space-y-3">
          ...
          <textarea value={form.notes || ''} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={3} className="input" placeholder="Internal notes..." />
          <h2 className="font-semibold pt-3">Terms & Conditions</h2>
          <textarea value={form.termsConditions || ''} onChange={(e) => setForm({ ...form, termsConditions: e.target.value })} rows={3} className="input" placeholder="Customer-facing T&C..." />
        </div>
        <div className="card p-5 space-y-3">

...
export default async function NewPurchaseOrderPage({ searchParams }: { searchParams: { vendorId?: string } }) {
  ...
  const last = await prisma.purchaseOrder.findFirst({ where: { organizationId: orgId }, orderBy: { createdAt: 'desc' } });
  const lastNum = last ? parseInt(last.orderNumber.replace(/\D/g, '')) : 4000;

  return (
    <div className="space-y-4">
      <Link href="/purchase-orders" className="text-sm text-ink-500 hover:text-ink-900 flex items-center gap-1">
      ...
      <TransactionForm
        ...
        nextNumber={`PO-${String(lastNum + 1).padStart(4, '0')}`}
        ...
        initial={searchParams.vendorId ? { contactId: searchParams.vendorId } : undefined}
      />
    </div>
  );
}

  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-ink-500">All payments received from customers.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card p-4"><p className="text-xs text-ink-500">Total Amount</p><p className="text-2xl font-bold mt-1 text-emerald-600">{formatCurrency(total)}</p></div>
      </div>
          ...
          <tbody className="divide-y divide-ink-200">
            ...
            {payments.map((p) => (
              <tr key={p.id} className="hover:bg-ink-50">
                ...
                <td className="px-4 py-2.5"><Link href={`/contacts/${p.customer.id}`} className="text-brand-600 hover:underline">{p.customer.companyName || `${p.customer.firstName} ${p.customer.lastName}`}</Link></td>
                <td className="px-4 py-2.5 text-ink-600">{p.invoice ? <Link href={`/invoices/${p.invoice.id}`} className="text-brand-600 hover:underline">{p.invoice.invoiceNumber}</Link> : '—'}</td>
                <td className="px-4 py-2.5 text-ink-600 capitalize">{p.paymentMode.replace('_', ' ')}</td>
                <td className="px-4 py-2.5 text-right font-medium text-emerald-600">{formatCurrency(p.amount, p.currencyCode)}</td>
              </tr>
            ))}
          </tbody>
        </table>
